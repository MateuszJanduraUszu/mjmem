# MJMEM documentation

## Getting started with MJMEM documentation

This document serves as a comprehensive guide for all MJMEM-related classes and functions.

## Documentation by categories

Navigate through the neatly categorized sections to gain insights into the different components of MJMEM:

### Memory allocators

- [allocator](reference/allocator/allocator.md)
- [block_allocator](reference/block_allocator/block_allocator.md)
- [dynamic_allocator](reference/dynamic_allocator/dynamic_allocator.md)
- [object_allocator](reference/object_allocator/object_allocator.md)
- [pool_allocator](reference/pool_allocator/pool_allocator.md)
- [synchronized_allocator](reference/synchronized_allocator/synchronized_allocator.md)

### Memory resources

- [pool_resource](reference/pool_resource/pool_resource.md)

### Allocator-aware memory management

- [allocate_object_array](reference/object_allocator/allocate_object_array.md)
- [allocate_object_array_using_allocator](reference/object_allocator/allocate_object_array_using_allocator.md)
- [create_object](reference/object_allocator/create_object.md)
- [create_object_using_allocator](reference/object_allocator/create_object_using_allocator.md)
- [delete_object](reference/object_allocator/delete_object.md)
- [delete_object_array](reference/object_allocator/delete_object_array.md)
- [delete_object_array_using_allocator](reference/object_allocator/delete_object_array_using_allocator.md)
- [delete_object_using_allocator](reference/object_allocator/delete_object_using_allocator.md)

### Exceptions

- [allocation_failure](reference/exception/allocation_failure.md)
- [allocation_limit_exceeded](reference/exception/allocation_limit_exceeded.md)
- [resource_overrun](reference/exception/resource_overrun.md)

### Global state

- [get_allocator](reference/allocator/get_allocator.md)
- [set_allocator](reference/allocator/set_allocator.md)

### Smart pointers

- [make_smart_array](reference/smart_pointer/make_smart_array.md)
- [make_smart_ptr](reference/smart_pointer/make_smart_ptr.md)
- [make_unique_smart_array](reference/smart_pointer/make_unique_smart_array.md)
- [make_unique_smart_ptr](reference/smart_pointer/make_unique_smart_ptr.md)
- [smart_array](reference/smart_pointer/smart_array.md)
- [smart_ptr](reference/smart_pointer/smart_ptr.md)
- [unique_smart_array](reference/smart_pointer/unique_smart_array.md)
- [unique_smart_ptr](reference/smart_pointer/unique_smart_ptr.md)

### Utilities

- [compatible_allocator](reference/object_allocator/compatible_allocator.md)
- [make_optimized_pair](reference/smart_pointer/make_optimized_pair.md)
- [optimized_pair](reference/smart_pointer/optimized_pair.md)
- [reference_counter](reference/smart_pointer/reference_counter.md)

## Quick reference: alphabetical index

For quick access to specific classes or functions, refer to the alphabetical index:

### A

- [allocate_object_array](reference/object_allocator/allocate_object_array.md)
- [allocate_object_array_using_allocator](reference/object_allocator/allocate_object_array_using_allocator.md)
- [allocation_failure](reference/exception/allocation_failure.md)
- [allocation_limit_exceeded](reference/exception/allocation_limit_exceeded.md)
- [allocator](reference/allocator/allocator.md)

### B

- [block_allocator](reference/block_allocator/block_allocator.md)

### C

- [compatible_allocator](reference/object_allocator/compatible_allocator.md)
- [create_object](reference/object_allocator/create_object.md)
- [create_object_using_allocator](reference/object_allocator/create_object_using_allocator.md)

### D

- [delete_object](reference/object_allocator/delete_object.md)
- [delete_object_array](reference/object_allocator/delete_object_array.md)
- [delete_object_array_using_allocator](reference/object_allocator/delete_object_array_using_allocator.md)
- [delete_object_using_allocator](reference/object_allocator/delete_object_using_allocator.md)
- [dynamic_allocator](reference/dynamic_allocator/dynamic_allocator.md)

### G

- [get_allocator](reference/allocator/get_allocator.md)

### M

- [make_optimized_pair](reference/smart_pointer/make_optimized_pair.md)
- [make_smart_array](reference/smart_pointer/make_smart_array.md)
- [make_smart_ptr](reference/smart_pointer/make_smart_ptr.md)
- [make_unique_smart_array](reference/smart_pointer/make_unique_smart_array.md)
- [make_unique_smart_ptr](reference/smart_pointer/make_unique_smart_ptr.md)

### O

- [object_allocator](reference/object_allocator/object_allocator.md)
- [optimized_pair](reference/smart_pointer/optimized_pair.md)

### P

- [pool_allocator](reference/pool_allocator/pool_allocator.md)
- [pool_resource](reference/pool_resource/pool_resource.md)

### R

- [reference_counter](reference/smart_pointer/reference_counter.md)
- [resource_overrun](reference/exception/resource_overrun.md)

### S

- [set_allocator](reference/allocator/set_allocator.md)
- [smart_array](reference/smart_pointer/smart_array.md)
- [smart_ptr](reference/smart_pointer/smart_ptr.md)
- [synchronized_allocator](reference/synchronized_allocator/synchronized_allocator.md)

### U

- [unique_smart_array](reference/smart_pointer/unique_smart_array.md)
- [unique_smart_ptr](reference/smart_pointer/unique_smart_ptr.md)